    ...
    <section class=" mt-10 rounded-lg shadow px-8 py-8 bg-white">
              <div class="flex  justify-between items-center gap-4 mb-8" >
                <div x-data="{ preview: null }">
                  <input type="file" wire:model="pdf" class="form-control" accept=".pdf" @change="preview = $event.target.files[0]" />
                  <button wire:click="uploadPdf" class="btn btn-primary" :disabled="!pdf">Upload PDF</button>
                  <div x-show="preview">
                      <img :src="URL.createObjectURL(preview)" alt="PDF Preview" />
                  </div>
              </div>
